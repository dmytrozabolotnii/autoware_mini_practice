...
class CameraTrafficLightDetector:
    ...
    def local_path_callback(self, local_path_msg):
        waypoints_xy = np.array(
            [(w.pose.pose.position.x, w.pose.pose.position.y) for w in local_path_msg.waypoints])
        # convert waypoints to shapely linestring
        local_path_linestring = LineString(waypoints_xy)
        stoplines_on_path = []

        for key, stopline in self.tfl_stoplines.items():
            if local_path_linestring.intersects(stopline):
                stoplines_on_path.append(key)
    ...
    def camera_image_callback(self, camera_image_msg):
        ...
        if transform_from_frame is None:
            rospy.logwarn_throttle(10, "%s - No transform received, skipping image", rospy.get_name())
            return
        # Generate Image
        ...
        # Process predictions to results
        tfl_status = TrafficLightResultArray(header=camera_image_msg.header)
    ...
    def create_roi_images(self, image, rois):
        ...
        for _, _, min_u, max_u, min_v, max_v in rois:
            ...
            if len(cropped_image) > 0:
                roi = cv2.resize(cropped_image, (128, 128), interpolation=cv2.INTER_LINEAR)
